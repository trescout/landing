# Dizi arşivinizi otomatik yönetin

Sonarr, haber grupları (newsgroup) ve bittorrent kullanıcıları için geliştirilmiş akıllı bir kişisel video kaydedici (PVR). Kullanıcıların dizi içeriklerini otomatik olarak takip etmesini, indirmesini ve düzenlemesini sağlayan açık kaynaklı bir medya yönetim aracıdır.

- ★ 16.274
- C#
- GitHub Trending · 2026-09-12

## Güncelleme
- 17 Eylül 2026: Yıldız 15.798 → 16.274, son sürüm v4.0.20.3014 (16 Eylül 2026).
- 12 Eylül 2026: Yıldız 15.796 → 15.798, son sürüm v4.0.19.2979 (26 Haziran 2026).

## Ne kazandırır?
- Yeni dizi bölümlerini otomatik olarak algılar ve indirir.
- Mevcut dosyalarınızı daha yüksek kaliteyle otomatik günceller.
- Medya dosyalarınızı düzenler ve isimlerini isteğinize göre değiştirir.

## Nasıl başlanır?

Resmî web sitesi üzerinden işletim sisteminize uygun kurulum dosyasını indirerek başlayabilirsiniz. Kurulum ve yapılandırma adımları için resmî dokümantasyon sayfasındaki indirme bağlantısını kullanın.
- [Resmî kaynak →](https://sonarr.tv)

- **Kimin için:** Dizi ve video içeriklerini düzenli bir şekilde takip etmek, indirmek ve arşivlemek isteyen medya tutkunları için uygundur. 
- **Lisans:** GPL-3.0 

## Bağlantılar
- [GitHub deposu →](https://github.com/Sonarr/Sonarr)

TreScout bu aracı geliştirmedi · GitHub trendlerinde keşfedip Türkçe tanıttı. Bu sayfa deponun 2026-09-12 tarihindeki hâlini anlatır: Yıldız sayısı ve yazdığımız metin o güne aittir, depo sonrasında değişmiş olabilir. Güncel durum için depo bağlantısına bakın.

## İlgili sözlük terimleri
Newsgroup PVR

---
Kaynak: TreScout Keşif · https://trescout.com/discover/sonarr/
TreScout her gün GitHub, Hacker News ve HuggingFace trendlerini Türkçe özetler.
