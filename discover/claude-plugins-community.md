# Yapay zekâ kodlama asistanına eklentiler ekleyin

Anthropic tarafından geliştirilen Claude Cowork ve Claude Code araçları için oluşturulan topluluk eklenti dizini, yazılım geliştirme süreçlerini otomatikleştiren genişletilebilir bir yapı sunuyor. Kullanıcılar, Python tabanlı bu eklentiler sayesinde yapay zekâ destekli kodlama asistanlarının yeteneklerini özelleştirebiliyor.

- ★ 1.046
- Python
- GitHub Trending · 2026-08-24

## Ne kazandırır?
- Kodlama süreçlerini otomatikleştiren eklentilere erişim sağlar
- Güvenlik taramasından geçmiş onaylı araçları listeler
- Yapay zekâ destekli asistanların yeteneklerini özelleştirir

## Nasıl başlanır?

Claude Cowork için eklentileri claude.com/plugins adresinden kurabilirsiniz. Claude Code için ise ilgili dokümantasyonda belirtilen komut satırı araçlarını kullanarak marketplace üzerinden eklenti ekleme ve yükleme işlemlerini gerçekleştirebilirsiniz.
- [Resmî kaynak →](https://claude.com/product/cowork)

- **Kimin için:** Claude Cowork ve Claude Code yazılım geliştirme araçlarını kullanan ve yapay zekâ asistanlarının işlevselliğini artırmak isteyen geliştiriciler için uygundur. 
- **Lisans:** Apache-2.0 

## Bağlantılar
- [GitHub deposu →](https://github.com/anthropics/claude-plugins-community)

TreScout bu aracı geliştirmedi · GitHub trendlerinde keşfedip Türkçe tanıttı. Bu sayfa deponun 2026-08-24 tarihindeki hâlini anlatır: Yıldız sayısı ve yazdığımız metin o güne aittir, depo sonrasında değişmiş olabilir. Güncel durum için depo bağlantısına bakın.

## İlgili sözlük terimleri
Artificial Intelligence

---
Kaynak: TreScout Keşif · https://trescout.com/discover/claude-plugins-community/
TreScout her gün GitHub, Hacker News ve HuggingFace trendlerini Türkçe özetler.
